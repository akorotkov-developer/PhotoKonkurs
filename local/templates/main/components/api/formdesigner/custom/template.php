...
?>
	<script type="text/javascript">
		...
		jQuery(document).ready(function ($) {
			...
			$("#<?=ltrim($modalId, '#')?>").on('hidden.bs.modal', function (e) {
...
<? if($arParams['USE_MODAL']): ?>
	...
	<button <?=($arParams['MODAL_BTN_ID'] ? 'id="' . $arParams['MODAL_BTN_ID'] . '"' : '')?>
		 class="<?=$arParams['MODAL_BTN_CLASS']?> btn btn-secondary"
		 onclick="jQuery.fn.apiModal('show',{id:'<?=$modalId?>'});"
         type="button"
         data-toggle="modal"
         data-target="#<?=ltrim($modalId, '#')?>"
    >
		<span class="<?=$arParams['MODAL_BTN_SPAN_CLASS']?>"></span><?=$arParams['MODAL_BTN_TEXT']?>
	</button>
    <? endif ?>
	<div id="<?=ltrim($modalId, '#')?>" class="modal fade modal-review" tabindex="-1" role="dialog" aria-hidden="true">

        <?/*<div id="modal<?=$modalId?>" tabindex="-1" role="dialog" aria-hidden="true" class="modal fade modal-review">*/?>
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <? if($arParams['MODAL_HEADER_TEXT']): ?>
                            <h5 class="modal-title" id="makeOrderLabel"><?=$arParams['MODAL_HEADER_TEXT']?></h5>
                        <? endif ?>
...
<? endif ?>
				...
				<div id="afd_wrap_<?=$formId;?>" class="api-formdesigner afd-theme-<?=$theme;?> afd-color-<?=$color;?>">
					...
					<form id="<?=$formId;?>"
					      ...
					      method="POST"
					      class="form afd-form <?=$arParams['FORM_HORIZONTAL'] ? 'afd-form-horizontal': ''?>"
						...
						if(!empty($arResult['DISPLAY_PROPERTIES'])) {
							foreach($arResult['DISPLAY_PROPERTIES'] as $arProp) {
								...
								$fieldError = '<small class="afd-error" data-bv-validator="notEmpty" data-bv-for="FIELDS[TITLE]" data-bv-result="INVALID" style=""></small>';
                                ...
                                $classFormGroup .= $fieldReq ? ' required' : '';
								...
								switch($arProp['PROPERTY_TYPE']) {
									...
									case 'S': // textarea + text + DateTime
										...
										<? elseif($arProp['USER_TYPE'] == 'HTML' || $arProp['USER_TYPE'] == 'TEXT'): ?>
											<div class="afd-row <?=$classFormGroup?> <?=$form_row_class?> <?=($arParams['WYSIWYG_ON'] ? 'afd-row-wysiwyg' : '')?>" data-field="<?=$propCode;?>">
												<? if($fieldName): ?>
													<div class="afd-label"><?=$fieldName;?></div>
												<? endif; ?>
												<div class="afd-controls">
										...
										<? elseif($arProp['USER_TYPE'] == 'DateTime' || $arProp['USER_TYPE'] == 'Date'): ?>
											<div class="afd-row <?=$classFormGroup?> form-group__date <?=$form_row_class?>" data-field="<?=$propCode;?>">
												<? if($fieldName): ?>
                                                    <label class="control-label" for="<?=$fieldId;?><?=$fieldMulti ? '_0' : '';?>"><?=$fieldName;?>&nbsp;</label>
												<? endif; ?>
												<?/*<div class="afd-controls">*/?>
													<? for($i = 0; $i < $fieldMultiCnt; $i++): ?>
                                                        <div class="calendar-icon">
															<input type="text"
															       ...
															       class="form-control date-control <?=$fieldClass;?> afd-field afd-type-text "
															       value="<?=$fieldMulti ? $arProp['USER_VALUE'][ $i ] : $arProp['USER_VALUE'];?>"<?=$fieldData;?>>
										...
										<? else: ?>
											<div class="afd-row <?=$classFormGroup?> <?=$form_row_class?>" data-field="<?=$propCode;?>">
												<? if($fieldName): ?>
                                                    <label for="<?=$fieldId?>" class="control-label <?= !empty($fieldReq) ? 'required' : ''?>"><?=$fieldName;?></label>
												<? endif; ?>
												<div class="afd-controls">
													<? for($i = 0; $i < $fieldMultiCnt; $i++): ?>
														<div class="afd-control">
									...
									case 'L':
										{
											?>
											<? if(!empty($arProp['DISPLAY_VALUE'])): ?>
											<div class="<?=$classFormGroup?> afd-row <?=$form_row_class?>" data-field="<?=$propCode;?>">
												<div class="form-group switch-parent afd-controls">
                                                    <? if($fieldName): ?>
                                                        <label class="control-label"><?=$fieldName;?></label>
                                                    <? endif; ?>
													<? if($arProp['LIST_TYPE'] == 'L' && !$fieldMulti): ?>
														<div class="afd-control selectize-input items full has-options has-items">
															<select name="<?=$fieldCode;?>"
															        class="<?=$fieldClass;?> form-control afd-field afd-type-select afd-select-single"
															        id="<?=$fieldId;?>" <?//=$size?> <?=$fieldData;?>>
																<? foreach($arProp['DISPLAY_VALUE'] as $k => $v): ?>
													...
													<? elseif($arProp['LIST_TYPE'] == 'L' && $fieldMulti): ?>
														<div class="afd-control <?=($fieldMulti ? 'afd-control-multiple' : '')?> selectize-input items full has-options has-items">
															<select name="<?=$fieldCode;?>"
															        class="<?=$fieldClass;?> form-control afd-field afd-type-select afd-select-multiple"
															        id="<?=$fieldId;?>"
															        multiple="" <?//=$size?> <?=$fieldData;?>>
																<? foreach($arProp['DISPLAY_VALUE'] as $k => $v): ?>
																	<option value="<?=$k;?>"<? if(($v['DEF'] == 'Y' && !$arResult['POST']) || (is_array($arProp['USER_VALUE']) && in_array($k, $arProp['USER_VALUE']))): ?> selected=""<? endif; ?>><?=$v['VALUE'];?></option>
											...
											<div class="afd-row <?=$classFormGroup?> <?=$form_row_class?>" data-field="<?=$propCode;?>">
                                                ...
                                                */?>
														...
														<? if($arProp['USER_TYPE'] == 'APIFD_ESList'): ?>
															...
															?>
															<select name="<?=$fieldCode;?>"
															        class="form-control <?=$fieldClass;?> afd-field afd-type-select afd-select-single <? if($bShowPicture): ?>afd-show-picture<? endif ?>"
															        id="<?=$fieldId;?>" <? //=$size?> <?=$fieldData;?>>
																<?=$chooseOption?>
														...
														<? else: ?>
															...
															<? if($arProp['LIST_TYPE'] == 'L' && !$fieldMulti): ?>
																<select name="<?=$fieldCode;?>"
																        class="form-control <?=$fieldClass;?> afd-field afd-type-select afd-select-single"
																        id="<?=$fieldId;?>" <? //=$size?> <?=$fieldData;?>>
																	<? foreach($arProp['DISPLAY_VALUE'] as $k => $v): ?>
															...
															<? elseif($arProp['LIST_TYPE'] == 'L' && $fieldMulti): ?>
																<select name="<?=$fieldCode;?>"
																        class="form-control <?=$fieldClass;?> afd-field afd-type-select afd-select-multiple"
																        id="<?=$fieldId;?>"
																        multiple="" <?=$size?> <?=$fieldData;?>>
																	<? foreach($arProp['DISPLAY_VALUE'] as $k => $v): ?>
																		<option value="<?=$k;?>"<? if(($v['DEF'] == 'Y' && !$arResult['POST']) || (is_array($arProp['USER_VALUE']) && in_array($k, $arProp['USER_VALUE']))): ?> selected=""<? endif; ?>><?=$v['VALUE'];?></option>
									...
									case 'F':
										{
											?>
											<div class="<?=$classFormGroup?> choose-file DOCS  afd-row afd-row-type-upload <?=$form_row_class?>  <?= $propCode == 'RELEASE_FILE' ? 'd-none' : ''?>" data-field="<?=$propCode;?>">
                                                <div class="form-control-wrapper">
                                                    <? if($fieldName): ?>
                                                        <label class="control-label col-lg-6"" for="<?=$fieldId;?>_file"><?=$fieldName?>
                                                    ...
                                                    <div class="afd-controls">
                                                        ...
                                                        <div class="api_upload" id="<?=$fieldId;?>">
                                                            ...
                                                            <div class="api_upload_drop d-none">
												...
												<script type="text/javascript">
													jQuery(document).ready(function ($) {
													    ...
													    var options = {
													        itemRemoveClass:  '.api_file_remove',
                                                        };
														...
														$('#<?=$fieldId;?>').apiUpload({
															...
															callback: {
                                                                ...
                                                                onFilesMaxError: function (item) {
																    $(item).find(options.itemRemoveClass).click();
                                                                }
															}
														});

														$('#<?=$fieldId;?>').on('click', options.itemRemoveClass, function () {
															var fileButton = $(this);
                        ...
                        <? endif */?>
						<? if($arParams['USE_EULA'] && $arParams['MESS_EULA']): ?>
							<div class="form-group col-12 afd-row afd-row-eula afd-row-accept">
								<div class="custom-control custom-checkbox afd-controls">
                                    <input type="checkbox" id="accept_field_<?=$formId?>" name="EULA_ACCEPTED" value="Y" class="custom-control-input api-field" <?=$arResult['EULA_ACCEPTED'] == 'Y' ? ' checked' : ''?>>
                                    <label class="custom-control-label afd-accept-label" for="accept_field_<?=$formId?>">
										<?=$arParams['MESS_EULA']?>
									</label>
								</div>
							</div>
						...
						<div class="afd-row form-group col-12 text-center">
                            <? if($arParams['MODAL_FOOTER_TEXT']): ?>
                                <p class="warning-buy">
                                    <small><?=$arParams['MODAL_FOOTER_TEXT']?></small>
                                </p>
                            <? endif ?>
                            <button type="submit"
                                    name="API_FD_SUBMIT"
                                    value="Y"
                                    class="btn btn-secondary <?=$arParams['SUBMIT_BUTTON_CLASS'];?><?=$BUTTON_SIZE?>">
